...
function renderColors() {
    colorsArr.map((color)=>{
        colorSchemeHtml += `
        <div id="color-bar" class="color-bar" style="background-color:${color.hex.value}" aria-label="${color.name.value} HexCode is ${color.name.closest_named_hex}">
        </div>
        `
        colorSchemeHexCode += `<div id="color-hex" class="color-hex one"><p>${color.hex.value}</p></div>`
    })
